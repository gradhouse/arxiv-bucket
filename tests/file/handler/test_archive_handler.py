...
# placeholder directory, this is not used directly in the tests as extraction is mocked
TEST_SCRATCH_DIRECTORY = '/'
...
def test_get_file_type_from_format(filename, expected_file_type, file_test_fixtures_directory):
    """
    Test the get_file_type_from_format method when the file is present.
    """
    full_path = os.path.join(file_test_fixtures_directory, filename)
...
def test_is_archive_format(filename, expected_is_archive_file, file_test_fixtures_directory):
    """
...
def test_is_archive_format_raise_file_not_found(file_test_fixtures_directory):
    """
...
def test_is_tar_format(filename, expected_is_tar_file, file_test_fixtures_directory):
    """
...
def test_is_tar_format_raise_file_not_found(file_test_fixtures_directory):
    """
...
def test_is_tgz_format(filename, expected_is_tgz_file, file_test_fixtures_directory):
    """
...
def test_is_tgz_format_raise_file_not_found(file_test_fixtures_directory):
    """
...
def test_is_gzip_format(filename, expected_is_gzip_file, file_test_fixtures_directory):
    """
...
def test_is_gzip_format_raise_file_not_found(file_test_fixtures_directory):
    """
...
def test_list_tar_contents(filename, expected_contents, file_test_fixtures_directory):
    """
...
def test_list_tar_contents_raise_file_not_found(file_path, file_test_fixtures_directory):
    """
    Test the _list_tar_contents when the file is not found or the path directs to a non-file.
    """
    # test directory case
    with pytest.raises(FileNotFoundError):
        ArchiveHandler._list_tar_contents(file_test_fixtures_directory)
...
def test_list_tar_contents_raise_file_type_not_tar(filename, file_test_fixtures_directory):
    """
...
def test_list_tgz_contents(filename, expected_contents, file_test_fixtures_directory):
    """
...
def test_list_tgz_contents_raise_file_not_found(file_path, file_test_fixtures_directory):
    """
    Test the _list_tgz_contents when the file is not found or the path directs to a non-file.
    """
    # test directory case
    with pytest.raises(FileNotFoundError):
        ArchiveHandler._list_tgz_contents(file_test_fixtures_directory)
    # test file not found case
    with pytest.raises(FileNotFoundError):
...
def test_list_tar_contents_raise_file_type_not_tgz(filename, file_test_fixtures_directory):
    """
...
def test_list_gzip_contents_gzip(filename, expected_contents, file_test_fixtures_directory):
    """
...
def test_list_gzip_contents_gzip_extra_field(filename, expected_contents, file_test_fixtures_directory):
    """
...
def test_list_gzip_contents_raise_file_not_found(file_path, file_test_fixtures_directory):
    """
    Test the list_gzip_contents when the file is not found or the path directs to a non-file.
    """
    # test directory case
    with pytest.raises(FileNotFoundError):
        ArchiveHandler._list_gzip_contents(file_test_fixtures_directory)
    # test file not found case
    with pytest.raises(FileNotFoundError):
...
def test_list_gzip_contents_raise_file_type_not_gzip(filename, file_test_fixtures_directory):
    """
...
def test_list_contents_tar(mocker):
    ...
    """
    mock_is_archive = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_archive_format', return_value=True)
    mock_is_tar = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tar_format', return_value=True)
    mock_is_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tgz_format', return_value=False)
    mock_is_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_gzip_format', return_value=False)
    mock_list_tar = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler._list_tar_contents', return_value=['file1.txt', 'file2.txt'])
...
def test_list_contents_tgz(mocker):
    ...
    """
    mock_is_archive = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_archive_format', return_value=True)
    mock_is_tar = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tar_format', return_value=False)
    mock_is_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tgz_format', return_value=True)
    mock_is_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_gzip_format', return_value=False)
    mock_list_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler._list_tgz_contents', return_value=['file1.txt', 'file2.txt'])
...
def test_list_contents_gzip(mocker):
    ...
    mock_is_tar = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tar_format', return_value=False)
    mock_is_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tgz_format', return_value=False)
    mock_is_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_gzip_format', return_value=True)
    mock_list_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler._list_gzip_contents', return_value=['file1.txt'])
...
def test_list_contents_unsupported_format(mocker):
    ...
    """
    mock_is_archive = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_archive_format', return_value=False)
...
def test_extract_gzip_contents_private_success(mocker):
    ...
    """
    mock_list_contents = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.list_contents')
    mock_gzip_open = mocker.patch('arxiv_bucket.file.handler.archive_handler.gzip.open')
    mock_path_join = mocker.patch('arxiv_bucket.file.handler.archive_handler.os.path.join')
    mock_open = mocker.patch('arxiv_bucket.file.handler.archive_handler.open')
...
def test_extract_tar_or_tgz_contents_private(mocker):
    ...
    This unit test is mocked and only tests the logic.
    """
    mock_tarfile_open = mocker.patch('arxiv_bucket.file.handler.archive_handler.tarfile.open')
...
def test_is_extract_possible_source_file_not_found(file_test_fixtures_directory):
    """
    ...
    source_file_path = file_test_fixtures_directory
    destination_directory = TEST_SCRATCH_DIRECTORY
    is_possible = ArchiveHandler.is_extract_possible(source_file_path, destination_directory)
    assert not is_possible

def test_is_extract_possible_destination_directory_not_found(file_test_fixtures_directory):
    """
    Test the is_extract_possible when the destination directory is not found.
    """

    # test case: destination directory not found
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.tar')
    ...
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.tar')
    destination_directory = os.path.join(file_test_fixtures_directory, 'top_level_text_file.txt')
    is_possible = ArchiveHandler.is_extract_possible(source_file_path, destination_directory)
    assert not is_possible

def test_is_extract_possible_contents_filenames_not_unique(mocker, file_test_fixtures_directory):
    """
    ...
    mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.list_contents', return_value=['file1.txt', 'File1.txt'])

    is_possible = ArchiveHandler.is_extract_possible(source_file_path, destination_directory)
    assert not is_possible

def test_is_extract_possible_contents_filename_traversal(mocker, file_test_fixtures_directory):
    """
    ...
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.tar')
    destination_directory = '/'
    mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.list_contents', return_value=['../file1.txt'])

    is_possible = ArchiveHandler.is_extract_possible(source_file_path, destination_directory)
    assert not is_possible

def test_is_extract_possible_destination_file_exists(mocker, file_test_fixtures_directory):
    """
    ...
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.tar')
    destination_directory = file_test_fixtures_directory
    mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.list_contents', return_value=['top_level_text_file.txt'])

    is_possible = ArchiveHandler.is_extract_possible(source_file_path, destination_directory)
    assert not is_possible

def test_is_extract_possible_success(mocker, file_test_fixtures_directory):
    """
    ...
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.tar')
    destination_directory = '/'
    mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.list_contents', return_value=['sample_contents.txt'])
...
def test_extract_contents_not_archive(mocker):
    ...
    """
    # set up the mocks
    mock_is_archive_format = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_archive_format')
    mock_is_gzip_format = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_gzip_format')
    mock_is_tar_format = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tar_format')
    mock_is_tgz_format = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tgz_format')
...
def test_extract_contents_gzip(mocker):
    ...
    """
    mock_is_archive = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_archive_format')
    mock_is_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_gzip_format')
    mock_is_tar = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tar_format')
    mock_is_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tgz_format')
    mock_extract_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.extract_gzip_contents')
    mock_extract_tar = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.extract_tar_contents')
    mock_extract_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.extract_tgz_contents')
...
def test_extract_gzip_contents_success(mocker, file_test_fixtures_directory):
    """
    ...
    """
    mock_is_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_gzip_format')
    mock_is_extract = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_extract_possible')
    mock_extract_gzip = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler._extract_gzip_contents_private')

    mock_is_gzip.return_value = True
    mock_is_extract.return_value = True

    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/gzip_single_file.gz')
    destination_directory = '/'
...
def test_extract_gzip_contents_not_gzip_format(mocker, file_test_fixtures_directory):
    """
...
def test_extract_gzip_contents_extract_not_possible(mocker, file_test_fixtures_directory):
    """
...
def test_extract_tgz_contents_success(mocker, file_test_fixtures_directory):
    """
    ...
    mock_is_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_tgz_format')
    mock_is_extract = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler.is_extract_possible')
    mock_extract_tar_or_tgz = mocker.patch('arxiv_bucket.file.handler.archive_handler.ArchiveHandler._extract_tar_or_tgz_contents_private')

    mock_is_tgz.return_value = True
    mock_is_extract.return_value = True

    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tgz_archive_file.gz')
    destination_directory = '/'
...
def test_extract_tgz_contents_not_tgz_format(mocker, file_test_fixtures_directory):
    """
...
def test_extract_tgz_contents_extract_not_possible(mocker, file_test_fixtures_directory):
    """
...
def test_extract_tar_contents_success(mocker, file_test_fixtures_directory):
    """
    ...
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.gz')
    destination_directory = '/'
...
def test_extract_tar_contents_not_tar_format(mocker, file_test_fixtures_directory):
    """
...
def test_extract_tar_contents_extract_not_possible(mocker, file_test_fixtures_directory):
    """
...
@pytest.mark.parametrize("source_file_path, destination_directory, expected_errors",
    [
        ...
        (os.path.dirname(__file__), '/', ['file not found']),
    ]
)
def test_check_extract_possible_file_or_dir_not_found_basic(source_file_path, destination_directory, expected_errors):
    """
    Test check_extract_possible for file or directory not found cases (basic, not using fixture directory).
    """
    errors = ArchiveHandler.check_extract_possible(source_file_path, destination_directory)
    assert errors == expected_errors


def test_check_extract_possible_file_or_dir_not_found_fixture_directory_not_found(file_test_fixtures_directory):
    """
    Test check_extract_possible for destination directory not found and destination is a file, using fixture directory.
    """
    # Destination directory not found
    source_file_path = os.path.join(file_test_fixtures_directory, 'archive/tar_archive_file.tar')
    destination_directory = 'non_existent_directory'
    expected_errors = ['destination directory not found']
    errors = ArchiveHandler.check_extract_possible(source_file_path, destination_directory)
    assert errors == expected_errors
...
def test_check_extract_possible_contents_filenames_not_unique(mocker, file_test_fixtures_directory):
    """
...
def test_check_extract_possible_contents_filename_traversal(mocker, file_test_fixtures_directory):
    """
...
def test_check_extract_possible_destination_file_exists(mocker, file_test_fixtures_directory):
    """
...
def test_check_extract_possible_success(mocker, file_test_fixtures_directory):
    """
