...
from arxiv_bucket.services.hash_service import HashType, HashService
...
def test_get_hash_encoder_instance_raise_hash_type_category_unknown():
    ...
    with pytest.raises(ValueError):
        HashService._get_hash_encoder_instance(cast(HashType, 1337))
...
def test_calculate_file_hash_raise_hash_type_unknown():
    ...
    with pytest.raises(ValueError):
        HashService.calculate_file_hash(valid_file_path, cast(HashType, 1337))

    with pytest.raises(ValueError):
        HashService.calculate_file_hash(valid_file_path, cast(HashType, 1337), file_buffer_size=valid_buffer_size)
...
def test_calculate_buffer_hash_raise_hash_type_unknown():
    ...
    try:
        ...
        with pytest.raises(ValueError):
            HashService.calculate_buffer_hash(buffer, cast(HashType, 1337))
    finally:
        os.remove(temp_file_path)
