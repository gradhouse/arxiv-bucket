...
def test_is_configured_success(monkeypatch):
    """
    Test that is_configured returns True when subprocess.run succeeds.
    """
    def fake_run(*a, **k):
        return 0
    monkeypatch.setattr("subprocess.run", fake_run)
    assert S3Client.is_configured() is True

def test_is_configured_failure(monkeypatch):
    """
    Test that is_configured returns False when subprocess.run raises an error.
    """
    def fake_run(*a, **k):
        raise subprocess.CalledProcessError(returncode=1, cmd="aws sts get-caller-identity")
    monkeypatch.setattr("subprocess.run", fake_run)
...
def test_copy_object_from_s3_invalid_dest():
    ...
    """
    with pytest.raises(ValueError):
        S3Client.copy_object_from_s3("s3://bucket/key", "")
...
def test_copy_object_from_s3_missing_directory(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr("arxiv_bucket.file.file_system.FileSystem.is_directory", lambda d: False)
...
def test_copy_object_from_s3_timeout(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("subprocess.run", fake_run)
    dest_file = tmp_path / "file.txt"
    monkeypatch.setattr("arxiv_bucket.file.file_system.FileSystem.is_directory", lambda d: True)
