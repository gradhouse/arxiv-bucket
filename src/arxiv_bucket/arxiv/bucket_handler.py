...
class BucketHandler:
    ...
    ARXIV_SOURCE_MANIFEST_FILENAME = 'arXiv_src_manifest.xml'
    ...
    def fetch_source_manifest(destination_directory: str) -> None:
        ...
        source_uri = f'{BucketHandler.ARXIV_SOURCE_S3_URI}{BucketHandler.ARXIV_SOURCE_MANIFEST_FILENAME}'
    ...
    def fetch_source_bulk_archive(bulk_archive_filename: str, destination_directory: str) -> None:
        ...
        if not BulkArchiveHandler.is_bulk_archive_filename(bulk_archive_filename):
            raise ValueError(f'Invalid bulk archive filename: {bulk_archive_filename}')

        source_uri = f'{BucketHandler.ARXIV_SOURCE_S3_URI}{bulk_archive_filename}'
        S3Client.copy_object_from_s3(source_uri, destination_directory)
